...
const validarCliente = yup.object().shape({
  ...
  telefone:yup.number().typeError('O campo preço deve ser preenchido com um valor'),
  genero_preferido:yup.string().required('O campo genero_preferido é obrigatório')
})
...
function EditarCliente() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:3005/clientes/${id}`)
      .then((response) => {
        reset(response.data);
      })
      .catch(() => {
  ...
  const { register, handleSubmit, formState: { errors }, reset } = useForm({ resolver: yupResolver(validarCliente) });

  const addCliente = data => axios.put(`http://localhost:3005/clientes/${id}`, data).then(() => {
