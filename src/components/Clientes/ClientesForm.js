...
function ClientesForm({btnText}) {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm({ resolver: yupResolver(validarCliente) });

  const addCliente = data => axios.post('http://localhost:3005/clientes', data)
  .then(() => {
